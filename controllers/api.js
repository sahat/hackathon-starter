...
exports.getFoursquare = (req, res, next) => {
  const token = req.user.tokens.find(tokens => tokens.kind === 'foursquare');
  Promise.all([
...
exports.getTumblr = (req, res, next) => {
  const token = req.user.tokens.find(tokens => tokens.kind === 'tumblr');
...
exports.getFacebook = (req, res, next) => {
  const token = req.user.tokens.find(tokens => tokens.kind === 'facebook');
...
exports.getScraping = (req, res, next) => {
  request.get('https://news.ycombinator.com/', (err, reqwest, body) => {
    if (err) { return next(err); }
...
exports.getNewYorkTimes = (req, res, next) => {
  ...
  request.get({ url: 'http://api.nytimes.com/svc/books/v2/lists', qs: query }, (err, reqwest, body) => {
    if (err) { return next(err); }
...
exports.getLastfm = (req, res, next) => {
  ...
  Promise.all([
    ...
    .then(([artistInformation, artistTopAlbumsInfo, artistTopTracksInfo]) => {
      const artist = {
        name: artistInformation.artist.name,
        image: artistInformation.artist.image.slice(-1)[0]['#text'],
        tags: artistInformation.artist.tags.tag,
        bio: artistInformation.artist.bio.summary,
        stats: artistInformation.artist.stats,
        similar: artistInformation.artist.similar.artist,
        topAlbums: artistTopAlbumsInfo,
        topTracks: artistTopTracksInfo,
      };
...
exports.getTwitter = (req, res, next) => {
  const token = req.user.tokens.find(tokens => tokens.kind === 'twitter');
  const T = new Twit({
...
exports.postTwitter = (req, res, next) => {
  ...
  return T.post('statuses/update', { status: req.body.tweet }, (err) => {
    if (err) { return next(err); }
...
exports.getSteam = (req, res, next) => {
  ...
  Promise.all([
    ...
    .then(([playerAchievementsResponse, playerSummariesResponse, ownedGamesResponse]) => {
      res.render('api/steam', {
        title: 'Steam Web API',
        ownedGames: ownedGamesResponse.response.games,
        playerAchievemments: playerAchievementsResponse.playerstats,
        playerSummary: playerSummariesResponse.response.players[0],
      });
    })
    .catch(next);
};
...
exports.postTwilio = (req, res, next) => {
  ...
  return twilio.sendMessage(message, (err, responseData) => {
...
exports.postClockwork = (req, res, next) => {
  ...
  clockwork.sendSms(message, (err, responseData) => {
    ...
    return res.redirect('/api/clockwork');
  });
};
...
exports.getLinkedin = (req, res, next) => {
  const token = req.user.tokens.find(tokens => tokens.kind === 'linkedin');
  const linkedin = Linkedin.init(token.accessToken);
  linkedin.people.me((err, $in) => {
    if (err) { return next(err); }
    return res.render('api/linkedin', {
      title: 'LinkedIn API',
      profile: $in,
    });
  });
...
exports.getInstagram = (req, res, next) => {
  const token = req.user.tokens.find(tokens => tokens.kind === 'instagram');
...
exports.getPayPal = (req, res, next) => {
  ...
  paypal.payment.create(paymentDetails, (err, payment) => {
    ...
    return next(new Error('Error while make a payment.'));
  });
};
...
exports.getPinterest = (req, res, next) => {
  const token = req.user.tokens.find(tokens => tokens.kind === 'pinterest');
  request.get({ url: 'https://api.pinterest.com/v1/me/boards/', qs: { access_token: token.accessToken }, json: true }, (err, reqwest, body) => {
    if (err) { return next(err); }
...
exports.postPinterest = (req, res, next) => {
  ...
  return request.post('https://api.pinterest.com/v1/pins/', { qs: { access_token: token.accessToken }, form: formData }, (err, reqwest, body) => {
    if (err) { return next(err); }
    if (reqwest.statusCode !== 201) {
