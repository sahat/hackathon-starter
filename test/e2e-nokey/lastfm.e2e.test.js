process.env.API_TEST_FILE = 'e2e-nokey/lastfm.e2e.test.js';
...
registerTestInManifest('e2e-nokey/lastfm.e2e.test.js');

// Skip this file during replay if it's not in the manifest
if (process.env.API_MODE === 'replay' && !isInManifest('e2e-nokey/lastfm.e2e.test.js')) {
  console.log('[fixtures] skipping e2e-nokey/lastfm.e2e.test.js as it is not in manifest for replay mode - 6 tests');
