...
test.describe('PubChem API Integration', () => {
  ...
  test('testing PubChem API endpoint directly', async ({ request, page }) => {
    await page.goto('/api/pubchem');
    await page.waitForLoadState('networkidle');
    ...
    const chemicalInfoCard = page.locator('.card.text-white.bg-success .card-header h6');
    await expect(chemicalInfoCard).toContainText('Chemical Information - Aspirin');

    // Test that Aspirin name is displayed
    await expect(page.locator('h4', { hasText: 'Aspirin' })).toBeVisible();

    // Test that at least one of the main sections is visible
    const propertiesCard = page.locator('.card.text-white.bg-info');
    const additionalInfoCard = page.locator('.card.text-white.bg-dark');

    const hasPropertiesCard = (await propertiesCard.count()) > 0;
    const hasAdditionalInfoCard = (await additionalInfoCard.count()) > 0;

    expect(hasPropertiesCard || hasAdditionalInfoCard).toBeTruthy();
  });
});
